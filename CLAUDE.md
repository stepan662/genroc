# gent

## Database

Queries are generated by **sqlc** from `internal/db/queries.sql`.
Generated code lives in `internal/db/gen/` (used at runtime, sqlite engine) and `internal/db/gen_pg/` (postgres validation artifact, never imported at runtime — but it must compile).

### Dual-engine requirement

The server supports both **SQLite** (default, single-file) and **PostgreSQL** (production).
All SQL in `queries.sql` must be compatible with both engines.

Rules:
- Use `BIGINT` (unix seconds) for all timestamp columns — works natively on both engines, no type overrides needed. Convert to/from `time.Time` using `toTime()` / `fromTimePtr()` in `db.go`.
- Use `sqlc.arg(name)` for all query parameters — never hardcode `?` or `$1`. sqlc translates this to the correct placeholder per engine.
- Both `gen/` and `gen_pg/` are committed. `go build ./...` compiles both and validates cross-engine compatibility without needing sqlc in CI.

Exceptions (hand-written in `db.go`, not in `queries.sql`):
- `ClaimInstances` — PostgreSQL uses `FOR UPDATE SKIP LOCKED` for concurrent workers; SQLite's single-writer model does not support this.
- `FindParentsOf` — dynamic-length `IN (...)` clause cannot be expressed in sqlc.

### Adding a query

1. Add to `internal/db/queries.sql` with annotation `-- name: QueryName :one/:many/:exec`
2. Run `sqlc generate` from repo root
3. Add a wrapper method on `*DB` in `db.go` if needed (e.g. to hide int64↔int conversions)

Install sqlc: `go install github.com/sqlc-dev/sqlc/cmd/sqlc@latest`

### Adding a migration

Create `internal/db/migrations/{NNN}_{description}.up.sql` (e.g. `002_add_column.up.sql`).
Use zero-padded 3-digit numbers — sqlc reads files in lexicographic order, so padding is required for correct schema ordering beyond 9 migrations. golang-migrate parses the prefix as an integer and is unaffected.
golang-migrate tracks applied migrations in the `schema_migrations` table.
sqlc reads the migrations directory directly, so no extra step is needed for query generation.

## Build / test

    make build      # produces ./gent and ./gentctl
    make test       # go test ./... + integration tests

    # Run with SQLite (default):
    ./gent -db gent.db

    # Run with PostgreSQL:
    ./gent -pg postgres://user:pass@localhost/gent

    # Run DB tests against PostgreSQL:
    POSTGRES_DSN=postgres://user:pass@localhost/gent go test ./internal/db/...
