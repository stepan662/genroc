...
type ErrorCase struct {
	Code       []string `json:"code,omitempty"        description:"Patterns matched against the error code. '%' is the only wildcard (matches any run of characters); every other character, including '_' and '.', is literal — so 'order_%' matches 'order_placed' but not 'order.placed'. Empty list = catch-all. Known engine codes — REST: http.NNN (e.g. http.500), http.timeout, pre.error, pre.timeout, output.parse, output.invalid; Script: script.N (exit code, e.g. script.1), script.timeout, pre.exec, output.parse; Child process: output.invalid. pre.* codes mean the call never reached the remote. On a child_map/child_list task the codes instead match the codes the child processes can raise, and each pattern is checked at registration against the child's raise set. A child that failed (rather than raised) is never catchable — convert the failure into a raise inside the child."`
