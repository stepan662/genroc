...
func TestHandle_PutAndListDefinitions(t *testing.T) {
	...
	payload, _ := json.Marshal(map[string]interface{}{
		...
		"steps": []map[string]interface{}{
			{"type": "task", "id": "s1", "transport": "http", "endpoint": "http://localhost/x"},
		},
	})
...
func TestHandle_ValidationErrors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			payload: `{"name":"p","version":1,"steps":[{"type":"task","id":"s1","transport":"http"}]}`,
			wantErr: "endpoint is required",
		},
		{
			name:    "unknown transport",
			payload: `{"name":"p","version":1,"steps":[{"type":"task","id":"s1","transport":"ftp","endpoint":"x"}]}`,
