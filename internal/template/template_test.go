...
// --- Splitting: where does a {{ }} block end? ---
//
...
// A single expression makes the whole template one chunk.
...
func TestParseErrorReportsLongestCandidate(t *testing.T) {
	...
	// `map(xs, y => {a: {b: 1}` is merely truncated. The real error — the missing
...
// A single expression preserves the value as given, rather than stringifying it.
func TestEvalAny_SingleExpressionPreservesInt(t *testing.T) {
	assertEvalAny(t, `{{ input.n }}`, 3)
}

func TestEvalAny_SingleExpressionPreservesBool(t *testing.T) {
...
func TestEvalAny_TwoExpressionsStringify(t *testing.T) {
...
// A single expression preserves its type.
func TestInferType_SingleExpressionPreservesType(t *testing.T) {
	assertInferType(t, `{{ input.n }}`, "integer")
}

// Mixed is always a string.
func TestInferType_MixedWithLiteralIsString(t *testing.T) {
	assertInferType(t, `n={{ input.n }}`, "string")
}

func TestInferType_TwoExpressionsIsString(t *testing.T) {
	assertInferType(t, `{{ input.name }}{{ input.n }}`, "string")
}

// A nullable interpolation in a mixed template would stringify to "null".
func TestInferType_RejectsNullableInMixedTemplate(t *testing.T) {
	assertInferRejects(t, `x={{ input.opt }}`)
}

// The same expression alone is fine — the null is preserved, not stringified.
func TestInferType_AllowsNullableAsWholeTemplate(t *testing.T) {
...
// engine's shapeRoots previously dropped SelfResult, so an externalized
...
// A single-expression template preserves the array a map produces; it is only
// mixed templates that must flatten to text.
func TestTemplateMapPreservesArray(t *testing.T) {
...
func TestTemplateMixedRejectsUnstringifiable(t *testing.T) {
...
// The same values are fine as a whole-template expression, where the type is
// preserved rather than stringified.
func TestTemplateWholeExpressionAllowsUnstringifiable(t *testing.T) {
	for _, c := range []struct{ name, src string }{
...
func TestTemplateMixedAllowsScalars(t *testing.T) {
	for _, c := range []struct{ name, src string }{
...
// unredacted.
func TestTemplateSecretThroughMapBodyTaints(t *testing.T) {
	if !mustParse(t, `{{ map(input.people, p => p.secret) }}`).ReferencesSecret(ctxSchema(t)) {
		t.Error("a secret on the element type must taint the template")
	}
}

func TestTemplateNonSecretThroughMapBodyDoesNotTaint(t *testing.T) {
